...
class PipelineController {
  public async create(req: Request, res: Response): Promise<Response> {
    try {
      const { name, description, funnel }: PipelineInterface = req.body;

      if (!name || !description || !funnel) return res.status(400).json({ message: 'Invalid value for pipeline' });

      // const existsPipeline = await Pipeline.findOne({ description });

      // if (existsPipeline) return res.status(400).json({ message: 'Pipeline already exists' });

      const pipeline = await Pipeline.create({ name, description, funnel }).save();
  ...
  public async update(req: Request, res: Response): Promise<Response> {
    try {
      const { name, description }: PipelineInterface = req.body;
      const id = req.params.id;
     ...
     //if (!name) return res.status(400).json({ error: 'Invalid value for pipeline' });
      ...
      await Pipeline.update(id, { name, description });
